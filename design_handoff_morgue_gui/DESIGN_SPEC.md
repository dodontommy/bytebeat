...
play/pitch/reverse/loop) · SURVIVOR: live · MIXER: live (fader/mute/meter) ·
HW/SYNC: live (note+CC) · TRANSPORT: live · SCOPE: live · STATUS: live ·
FIELD MANUAL: live.
...
## R1. STEP SAMPLER — "GRAIN LICKS" (drum machine / Channel Rack equivalent) — PLANNED

The single most-requested workflow: **record a wav, then sequence it into a
repeating rhythm** — like FL's Channel Rack.

- A new workspace (or a tab inside GRAIN MASS): a sample-sequencer grid.
- **8 sample slots**, each like a RACK voice. A slot holds ONE loaded sample
  plus a 16-step pattern.
- **Load** per slot (file chooser, same audio formats as GRAIN MASS).
- **16 step cells per slot** — click a cell and the sample fires on that
  step, in sync with the global BPM grid (the engine's existing clock).
- **Per-slot controls**: step pitch (semitones per step — so a kick can
  "fall"), velocity, level, and one-short the sample (re-trigger from start
  each hit) vs. choke (stop other samples in the same group — a drum-choke
  group).
- **Mute / solo** per slot, like the mixer.
- Renders into the same master bus so REC captures it, SURVIVOR can loop it.
- Reuses `SamplerVoice` but with its `pos` reset to 0 on each step-trigger;
  the engine's step clock (`bb.seq_pos`) is the sync source.

Design: slot = vertical strip (sample name, mute, 16 cells, pitch/vel knobs
folded into the cell grid). Matches RACK + MIXER visual grammar.
